...
<div class="relative">
    <div>
        <a href="{{ $link }}" class="block px-4 py-2 text-sm hover:bg-slate-200" role="menuitem" tabindex="-1" id="{{$menuName}}-item-{{$elementId}}">{{$text}}</a>
    </div>
    @if(isset($subMenu))
    <div id="{{$menuName}}-item-{{$elementId}}-menu" class="hidden origin-top-right absolute left-full top-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none galoo-submenu-submenu" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
