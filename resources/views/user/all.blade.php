...
<div class="flex items-center justify-between mb-6">
    ...
    <button
        class="flex items-center px-3 py-2 text-white transition duration-150 bg-black rounded-full hover:opacity-100 hover:outline-none hover:ring-2 hover:ring-offset-2 hover:ring-offset-white-800 hover:ring-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white-800 focus:ring-black"
        id="addUser">
            ...
            @foreach ($users as $user)
                <tr>
                    <td class="flex justify-center"><img
                            src="{{ isset($user->image) ? asset($user->image) : asset('images/uploads/profiles/no_image/no_image.jpg') }}"
                            alt="" class="object-cover object-center w-24 h-24 rounded-full"></td>
                    <td>{{ $user->name }}</td>
                    <td>{{ $user->surname }}</td>
                    <td>{{ $user->username }}</td>
                    <td>
                        <div class="flex content-center justify-center gap-4">
                            <a href="{{ route('user.view', ['id' => $user->id]) }}" class="text-blue-600">
                                <x-button.view />
                            </a>
                            <a href="{{ route('user.edit', ['id' => $user->id]) }}" class="text-amber-600">
                                <x-button.edit />
                            </a>
                            @if (session()->get('user')->id != $user->id && ($user->role != 'super' && $user->role != 'editor'))
                                <a href="#" onClick="deleteUser(this, {{ $user->id }});" class="text-red-600">
                                    <x-button.delete />
                                </a>
                            @endif
                        </div>
    ...
    $(document).ready(function() {
        $('#addUser').on('click', function() {
            Swal.fire({
                ...
                html: `<div class="grid grid-cols-2 gap-4 p-2 text-left md:grid-cols-6 auto-cols-fr">
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            let urlText = `<div class="flex content-center justify-center gap-4">
                                <a href="${url1.substring(0, url1.length - 1)}${response}" class="text-blue-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                                        <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" />
                                    </svg>
                                </a>
                                <a href="${url2.substring(0, url2.length - 1)}${response}" class="text-amber-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                    </svg>
                                </a>
                                <a href="#" onClick="deleteUser(this,${response});" class="text-red-600 delete-link">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
