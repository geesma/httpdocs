...
<div class="flex items-center justify-between mb-6">
  <h1 class="text-5xl font-medium leading-tight">Usuarios</h1>
  <button class="flex items-center px-3 py-2 text-white transition duration-150 bg-black rounded-full hover:opacity-100 hover:outline-none hover:ring-2 hover:ring-offset-2 hover:ring-offset-white-800 hover:ring-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white-800 focus:ring-black" id="addUser">
    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
      <path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd" />
    </svg>
    <span class="hidden ml-2 md:block">Añadir Usuario</span>
  </button>
</div>
<div>
  <table class="table-auto" id="users-table">
    ...
    <tbody>
      @foreach ($users as $user)
        <tr>
          <td class="flex justify-center"><img src="{{ isset($user->image) ? asset($user->image) : asset('images/uploads/profiles/no_image/no_image.jpg') }}" alt="" class="object-cover object-center w-24 h-24 rounded-full"></td>
          <td>{{ $user->name }}</td>
          <td>{{ $user->surname }}</td>
          <td>{{ $user->username }}</td>
          <td>
  ...
  $(document).ready(function() {
    $('#addUser').on('click', function() {
      Swal.fire({
        ...
        html:
          '<div class="grid grid-cols-2 gap-4 p-2 text-left md:grid-cols-6 auto-cols-fr"> ' +
            '<div class="col-span-2"> ' +
              '<label class="block mb-3 ml-1" for="name">Nombre</label> ' +
              `<x-input.no-label placeholder="Nombre" id="name" name="name"/>` +
            '</div>' +
            '<div class="col-span-4"> ' +
              '<label class="block mb-3 ml-1" for="surname">Apellidos</label> ' +
        ...
        if (result.isConfirmed) {
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              let urlText= '<div class="flex content-center justify-center gap-4"><a href="'+url1.substring(0, url1.length - 1)+response+'" class="text-blue-600"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z" /><path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" /></svg></a>' +
                  '<a href="'+url2.substring(0, url2.length - 1)+response+'" class="text-amber-600"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg></a>' +
                  '<a href="#" onClick="deleteUser(this, '+response+');" class="text-red-600 delete-link"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" /></svg></a></div>';
              table.row.add( [
                '<img src="{{asset('images/uploads/profiles/no_image/no_image.jpg')}}" alt="" class="object-cover object-center w-24 h-24 mx-auto rounded-full">',
                name,
                surname,
                username,
                urlText
