...
<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4" aria-route="{{ route('temporada.index') }}">
    @foreach ($temporadas as $temporada)
        <?php $i++; ?>
        <x-temporada.card :id="$temporada->id" :link='route('temporada.liga.index', ['temporada' => $temporada])' title="{{ $temporada->nom_temporada }}"
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            const content = '<a href="#" aria-id="' + response.id +
                                '" aria-name="' + response.nom_temporada +
                                '" class="relative block max-w-sm p-6 bg-white border border-gray-100 rounded-lg shadow-md hover:bg-gray-100">' +
                                '<h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900">' +
                                response.nom_temporada + '</h5>' +
                                '<p class="font-normal text-gray-700">' + response
                                .content + '</p>' +
